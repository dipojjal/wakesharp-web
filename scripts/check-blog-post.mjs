#!/usr/bin/env node
/**
 * Check blog posts against the authoring contract in docs/blog-schedule.md
 ...
 *   npx tsx scripts/check-blog-post.mjs --all     # every post; what `npm run verify` runs
 *
...
/** Every post under src/content/blog, translations included. */
const allPosts = () => {
  const out = [];
  (function walk(dir) {
...
/** The contract's problems with one post, as readable sentences. */
...
const files = args.includes('--all') ? allPosts() : args;
if (!files.length) {
  console.error('usage: npx tsx scripts/check-blog-post.mjs <post.md> [more.md…] | --all');
  process.exit(2);
}
let failed = 0;
for (const file of files) {
  const problems = checkPost(file);
...
if (failed) {
  console.error(`\n  ${failed} of ${files.length} post(s) break the contract in docs/blog-schedule.md.\n`);
  process.exit(1);
}
console.log(`\n  OK: ${files.length === 1 ? files[0] : `${files.length} posts`}\n`);
